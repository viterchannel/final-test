  ...
  try {
    ...
    for (let i = 0; i < pings.length; i += CHUNK) {
      ...
      try {
        ...
        /* G4: A 400 Bad Request (not spoof-related) means the server considers
           these pings permanently invalid — retrying them forever would create
           an infinite loop. Drop them from the queue and log so the issue is
           visible in dev tools without blocking subsequent chunks. */
        const httpStatus =
          (err.status as number | undefined) ??
          (err.statusCode as number | undefined) ??
          (err.httpStatus as number | undefined);
        if (httpStatus === 400) {
          if (import.meta.env.DEV) {
            console.warn("[gpsQueue] Dropping GPS batch — server rejected with 400:", err);
          }
          await clearQueue(chunk.map(p => p.id));
          continue;
        }
