...
export default function Home() {
  ...
  const doActualToggle = async () => {
    if (!navigator.onLine) {
      showToast(T("offline") + " — " + T("liveUpdatesPaused"), "error");
      return;
    }
    const now = Date.now();
  ...
  /* Always-current ref so dismiss callbacks never close over stale requestsData.
     Rapid socket events can trigger multiple state updates before React flushes
     them — reading from a ref guarantees the latest server list. */
  const requestsDataRef = useRef(requestsData);
  useEffect(() => {
    requestsDataRef.current = requestsData;
  }, [requestsData]);
  ...
  const dismiss = useCallback(
    (id: string) => {
      ...
      setDismissed((prev) => {
        ...
        /* Read latest requestsData from the ref — avoids stale closure over
           allOrders/allRides that were captured at render time, which can cause
           incorrect "no remaining requests" judgements during rapid socket events. */
        const latestData = requestsDataRef.current;
        const latestOrders: any[] = latestData?.orders || [];
        const latestRides: any[] = latestData?.rides || [];
        const serverIds = new Set<string>([
          ...latestOrders.map((o: any) => o.id),
          ...latestRides.map((r: any) => r.id),
        ]);
