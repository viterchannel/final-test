...
async function isVpnOrProxy(ip: string): Promise<boolean> {
  ...
  try {
    ...
    /* Always use HTTPS to protect IP addresses in transit.
       With a key: pro.ip-api.com (HTTPS, paid, full VPN/proxy/hosting fields)
       Without key: proxycheck.io free tier (HTTPS, no key required) */
    let isVpn = false;
    if (ipApiKey) {
      const resp = await fetch(`https://pro.ip-api.com/json/${encodeURIComponent(ip)}?key=${ipApiKey}&fields=status,proxy,hosting`, {
        signal: AbortSignal.timeout(5_000),
      });
      if (!resp.ok) {
        logger.warn(`[VPN] pro.ip-api check failed for IP ${ip}: HTTP ${resp.status}`);
      ...
      /* proxycheck.io free tier — HTTPS, no key required, returns { status, proxy } */
      const resp = await fetch(`https://proxycheck.io/v2/${encodeURIComponent(ip)}?vpn=1&risk=1`, {
        signal: AbortSignal.timeout(5_000),
      });
      if (!resp.ok) {
        logger.warn(`[VPN] proxycheck.io failed for IP ${ip}: HTTP ${resp.status}`);
        addSecurityEvent({ type: "vpn_check_failed", ip, details: `VPN check HTTP error ${resp.status}`, severity: "low" });
        return false;
      }
      interface ProxyCheckResponse { status?: string; [ip: string]: unknown; }
      const data = await resp.json() as ProxyCheckResponse;
      const ipData = data[ip] as { proxy?: string; type?: string } | undefined;
      isVpn = data.status === "ok" && (ipData?.proxy === "yes");
    }
...
setInterval(() => {
  loadBlockedIPs().catch((e: Error) => logger.warn({ err: e.message }, "[security] blockedIP periodic refresh failed"));
}, 60 * 1000);
