...
/* Periodically purge expired OTP attempt rows from the DB so the table doesn't grow unbounded */
setInterval(async () => {
  try {
    await db.delete(otpAttemptsTable).where(sql`expires_at < now()`);
  } catch (e: unknown) {
    logger.warn("[rider] otp_attempts cleanup failed:", (e as Error)?.message);
  }
}, 5 * 60_000);
...
router.post("/orders/:id/accept", async (req, res) => {
  ...
  await db.transaction(async (tx) => {
    ...
    const [result] = await tx
      .update(ordersTable)
      .set({ riderId, riderName: String(riderUser.name || "Rider"), riderPhone: riderUser.phone ? String(riderUser.phone) : null, assignedRiderId: riderId, assignedAt: now, acceptedAt: now, updatedAt: now })
...
router.get("/history", async (req, res) => {
  ...
  const [orders, rides] = await Promise.all([
    db.select().from(ordersTable).where(and(eq(ordersTable.riderId, riderId), eq(ordersTable.status, "delivered"))).orderBy(desc(ordersTable.updatedAt)).limit(limitParam + 1).offset(offsetParam),
    db.select().from(ridesTable).where(and(eq(ridesTable.riderId, riderId), or(eq(ridesTable.status, "completed"), eq(ridesTable.status, "cancelled")))).orderBy(desc(ridesTable.updatedAt)).limit(limitParam + 1).offset(offsetParam),
  ]);
