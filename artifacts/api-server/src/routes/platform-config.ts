...
import { faqsTable, abExperimentsTable, abAssignmentsTable, termsVersionsTable } from "@workspace/db/schema";
...
/* ── GET /platform-config/terms-text?policy=terms — Public: fetch current terms body ── */
router.get("/terms-text", async (req, res) => {
  const policy = (req.query.policy as string) || "terms";
  try {
    const rows = await db
      .select({
        version: termsVersionsTable.version,
        bodyMarkdown: termsVersionsTable.bodyMarkdown,
        effectiveAt: termsVersionsTable.effectiveAt,
      })
      ...
      .limit(1);
    if (!rows.length) {
      sendNotFound(res, "No terms version found for this policy");
      return;
    }
    const r = rows[0];
    sendSuccess(res, {
    ...
    sendError(res, "Failed to fetch terms text");
  }
});
