      ...
      <ScrollView contentContainerStyle={ss.content}>
        ...
        <View style={ss.inputRow}>
          ...
          <TextInput
            ...
            value={travelDate}
            onChangeText={v => { setTravelDate(v); setDateError(null); }}
            onBlur={() => {
              const iso8601 = /^\d{4}-\d{2}-\d{2}$/;
              if (!iso8601.test(travelDate)) { setDateError("Enter a valid date (YYYY-MM-DD)"); return; }
              const d = new Date(travelDate + "T00:00:00");
              if (isNaN(d.getTime())) { setDateError("This is not a valid calendar date"); return; }
              const today = new Date(); today.setHours(0, 0, 0, 0);
              if (d < today) { setDateError("Travel date cannot be in the past"); return; }
              setDateError(null);
            }}
        ...
        {dateError && (
          <Text style={{ fontFamily: Font.regular, fontSize: 12, color: "#EF4444", marginBottom: 8, marginLeft: 4 }}>
            {dateError}
          </Text>
        )}
        <TouchableOpacity
          activeOpacity={0.7}
          style={[ss.btnPrimary, (loading || !!dateError) && ss.btnDisabled]}
          ...
          onPress={() => {
            ...
            const today = new Date(); today.setHours(0, 0, 0, 0);
            if (d < today) { setDateError("Travel date cannot be in the past"); return; }
            checkAvailability(selectedSchedule.id, travelDate);
          }}>
