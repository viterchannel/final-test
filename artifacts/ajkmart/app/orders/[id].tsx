...
export default function OrderDetailScreen() {
  ...
  const { id: routeId, action } = useLocalSearchParams<{ id: string; type?: string; action?: string }>();
  ...
  /* Auto-trigger refund when navigated with action=refund (e.g. from deep link or notifications) */
  useEffect(() => {
    if (action !== "refund" || !order || actionTriggeredRef.current) return;
    const isDeliveredNow = order.status === "delivered" || order.status === "completed";
    const isCashNow = order.paymentMethod === "cod" || order.paymentMethod === "cash";
    const hasRefundNow = order.refundStatus === "requested" || order.refundStatus === "approved" || order.refundStatus === "refunded";
    if (isDeliveredNow && !isCashNow && !refundRequested && !hasRefundNow) {
      actionTriggeredRef.current = true;
      setRefundRequesting(true);
      refundCall.execute().finally(() => {
        if (mountedRef.current) setRefundRequesting(false);
      });
    }
  }, [action, order, refundRequested]);
