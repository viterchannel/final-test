  ...
  const handleBarCodeScanned = async ({ type, data }: { type: string; data: string }) => {
    ...
    if (isUrl) {
      try {
        ...
        const platformDomain = process.env.EXPO_PUBLIC_DOMAIN?.trim() ?? "";
        const isOwnDomain = platformDomain && (url.hostname === platformDomain || url.hostname.endsWith(`.${platformDomain}`));
        if (isOwnDomain) {
...
const s = StyleSheet.create({
  ...
  permTitle: { fontFamily: Font.bold, fontSize: 22, color: Colors.light.text, textAlign: "center" },
  permSub: { fontFamily: Font.regular, fontSize: 14, color: Colors.light.textSecondary, textAlign: "center", lineHeight: 22 },
  permBtn: {
  ...
  backBtnTxt: { fontFamily: Font.medium, fontSize: 14, color: Colors.light.textSecondary },
});
